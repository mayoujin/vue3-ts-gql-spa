...
const isReactFragment = (node) => {
  ...
  if (t.isMemberExpression(element)) {
    return (
      element.object.name === 'React' && element.property.name === 'Fragment'
    )
  }
...
module.exports = (babel, pluginOptions = {}, ...args) => {
  ...
  if (options.target === 'vue:3') {
    moduleName = '@reshadow/vue3'
    KEYS.__classProp__ = 'class'
  }
  ...
  const traverseStyled = (p, { quasi } = {}) => {
    ...
    const variables =
      quasi && quasi.expressions.length && prepareExpressions(quasi, hash)
        ...
        JSXElement(elementPath) {
          ...
          if (openingElement.attributes.length > 0) {
            ...
            if (options.target.startsWith('vue')) {
              ...
              if (!require(moduleName).areAllAttributesStatic(openingElement.attributes)) {
                return
              }
              filtered.push(
                ...spreads.map((attr) => {
          ...
          if (options.target.startsWith('vue')) {
            openingElement.attributes.push(
              t.JSXAttribute(
                t.JSXIdentifier(KEYS.__static__)
              )
            );
          }
    ...
    if (options.target === 'vue') {
      ...
      p.traverse({
        JSXElement(elementPath) {
          transformVueJSX.visitor.JSXElement.exit(elementPath);
          // transformVueJSX.visitor.JSXElement(elementPath)
        },
        CallExpression(expressionPath) {
          const { node } = expressionPath
          /* normalize `attrs` for Vuejs 3 data hydrate function format */
          normalizeAttrs(node)

          if (!isHydrateExpression(node)) {
            return
          }
          addImport(node.callee.name)

          if (hasStaticStyle(node)) {
            return
          }
    ...
    if (options.target === 'vue:3') {
      ...
      const elems = [...elementMap.values()]
      const transformVueJSX = transformJSX(babel)

      p.traverse({
        JSXElement(elementPath) {
          transformVueJSX.visitor.JSXElement.exit[0](elementPath)
        },
      ...
      if (isStyledExpression(p.node)) {
        p.replaceWith(
          t.CallExpression(p.node, [
            t.CallExpression(t.identifier(addImport('__extract__')), []),
          ]),
        )
      }
    },
    ...
    visitor: _objectSpread(
      {
        ...
        ImportDeclaration(p) {
          ...
          if (cssFileRe && cssFileRe.test(source.value)) {
            ...
            const code = fs
              .readFileSync(file)
              .toString() // escape backticks and backslashes
              .replace(/[`\\]/g, '\\$&')
            const append = t.taggedTemplateExpression(
              t.identifier(addImport('css')),
              t.templateLiteral(
